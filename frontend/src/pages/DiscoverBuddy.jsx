...
const Dashboard = () => {
  ...
  useEffect(() => {
    api.get("/discover/food").then((r) => setFood(r.data.items || [])).catch(() => { });
  ...
  return (
    <div className="mx-5 mt-4 space-y-3">
      ...
      <Card>
        ...
        {food.length === 0 ? (
          <p className="text-xs text-slate-400 text-center py-3">Loading recommendations...</p>
        ) : (
          <div className="mt-3 grid grid-cols-3 gap-2" data-testid="food-grid">
            {food.slice(0, 3).map((f, i) => (
              <div key={i} className="rounded-xl overflow-hidden bg-slate-50 p-2">
...
const Food = () => {
  ...
  return (
    <div className="mx-5 mt-4 space-y-3">
      ...
      <Card>
        <h3 className="font-display font-bold text-base">Recommended for You</h3>
        {loading ? (
          <div className="mt-3 grid grid-cols-2 gap-2" data-testid="food-list">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="rounded-xl bg-slate-100 p-3 animate-pulse">
                <div className="h-3 bg-slate-200 rounded w-3/4 mb-2" />
                <div className="h-2 bg-slate-200 rounded w-1/2 mb-1" />
                <div className="h-2 bg-slate-200 rounded w-2/3" />
              </div>
            ))}
          </div>
        ) : error ? (
          <div className="mt-3 p-4 rounded-xl bg-rose-50 border border-rose-100 text-center">
            <p className="text-xs text-rose-600">{error}</p>
            <button onClick={fetchFood} className="mt-2 px-4 py-1.5 rounded-lg bdy-bg text-white text-xs font-semibold">Try Again</button>
          </div>
        ) : items.length === 0 ? (
          <p className="text-xs text-slate-400 text-center py-4">No recommendations available</p>
        ) : (
          <div className="mt-3 grid grid-cols-2 gap-2" data-testid="food-list">
            {items.map((f, i) => (
              <div key={i} className="rounded-xl overflow-hidden bg-slate-50 p-3">
                <div className="text-[12px] font-bold truncate">{f.name}</div>
                <div className="flex items-center gap-1 mt-0.5">
                  <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                  <span className="text-[10px] text-slate-500">{f.rating} · ₹{f.price} · {f.distance}</span>
                </div>
                <div className="text-[10px] text-slate-400 mt-1">{f.tag} · {f.dietary}</div>
                {f.reason && <div className="text-[10px] text-slate-500 mt-1 italic">{f.reason}</div>}
              </div>
            ))}
          </div>
        )}
...
const Travel = () => {
  ...
  const compareRoutes = async () => {
    ...
    setError(null);
    try {
      const response = await api.post("/discover/routes", { from: fromPlace, to: toPlace });
  ...
  return (
    <div className="mx-5 mt-4 space-y-3">
      ...
      <Card>
        ...
        {loading && (
          <div className="flex items-center justify-center gap-2 mt-3 py-4" data-testid="travel-loading">
            <div className="w-4 h-4 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin"></div>
            <span className="text-xs text-slate-500">Comparing routes...</span>
          </div>
        )}

        {error && !loading && (
          <div className="mt-3 p-3 rounded-xl bg-rose-50 text-center" data-testid="travel-error">
            <p className="text-xs text-rose-600 font-medium">{error}</p>
            <button onClick={compareRoutes} className="mt-2 px-3 py-1.5 rounded-lg bg-rose-100 text-rose-700 text-xs font-semibold hover:bg-rose-200">
              Try Again
            </button>
          </div>
        )}
      ...
      <Card>
        ...
        {places.length > 0 ? (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {places.map(p => (
              <span key={p.id} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-50 text-xs font-semibold text-slate-700 border border-slate-200 cursor-pointer hover:bg-slate-100"
                onClick={() => { if (!fromPlace.trim()) setFromPlace(p.name); else setToPlace(p.name); }}
                title="Click to fill input">
                {p.name}
                <button onClick={(e) => { e.stopPropagation(); deletePlace(p.id); }} className="text-slate-400 hover:text-rose-500 ml-0.5" aria-label={`Remove ${p.name}`}>×</button>
              </span>
            ))}
          </div>
        ) : (
