...
## Session: June 15, 2026 — AWS Deployment (Full Stack Live)

### What Was Done
- Deployed PocketBuddy to AWS free tier: EC2 (backend) + S3 (frontend) + nginx (reverse proxy)
- Resolved multiple issues: Security Group port 8000 blocked (switched to port 80), SSH key permissions on Windows, S3 DNS resolution on mobile carriers
- Final architecture: nginx serves React frontend from `/var/www/frontend/` and proxies `/api/*` to uvicorn on port 8000 — all from a single IP

### Infrastructure Created
| Resource | Details |
|----------|---------|
| EC2 Instance | `i-02c09f403e344c062`, t3.micro, Amazon Linux 2023, ap-southeast-2 (Sydney) |
| Public IP | `54.206.59.45` |
| S3 Bucket | `pocketbuddy-frontend-demo` (ap-southeast-2), static website hosting enabled |
| systemd Service | `pocketbuddy.service` — auto-starts uvicorn on boot |
| nginx | Reverse proxy on port 80 → frontend static + API proxy to :8000 |

### Files Created
| File | Description |
|------|-------------|
| `deployment/README.md` | Architecture diagram, cost summary, prerequisites |
| `deployment/STEP_BY_STEP.md` | Full walkthrough from zero to deployed (5 phases) |
| `deployment/WORKFLOW.md` | Cheat sheet for updating frontend/backend after code changes |
| `deployment/ec2-setup.sh` | One-shot EC2 instance setup script |
| `deployment/nginx.conf` | Nginx config with SSE streaming support |
| `deployment/pocketbuddy.service` | systemd service file for auto-restart |
| `deployment/deploy-frontend.ps1` | S3 bucket creation + upload script |
| `deployment/update-frontend.ps1` | Quick frontend re-deploy script |
| `deployment/update-backend.sh` | Quick backend re-deploy script (run on EC2) |

### Files Modified
| File | Changes |
|------|---------|
| `frontend/.env` | Changed `REACT_APP_BACKEND_URL` from `localhost:8000` to `http://54.206.59.45` |
| `frontend/public/index.html` | Removed "Made with Emergent" badge, Emergent script, PostHog analytics, DOMException handler |
| `.gitignore` | Added `*.pem`, `.ssh/`, `deployment/.env`, `deployment/bucket-policy.json` |
| `memory/PRD.md` | Updated with AWS deployment architecture, live URLs, expanded feature table |

### Live URLs
| What | URL |
|------|-----|
| Full App (frontend + backend) | http://54.206.59.45 |
| API Documentation (Swagger) | http://54.206.59.45/docs |
| S3 Frontend (backup) | http://pocketbuddy-frontend-demo.s3-website-ap-southeast-2.amazonaws.com |

### Issues Encountered & Resolved
1. **Port 8000 blocked externally** — Security Group had the rule but traffic still timed out. Resolved by running on port 80 instead (nginx → uvicorn)
2. **SSH key permissions on Windows** — Required removing all users except current user via `icacls` (Administrators, SYSTEM, Authenticated Users all had to be stripped)
3. **S3 URL not loading on mobile** — Long S3 hostname caused DNS resolution failures on some mobile carriers. Resolved by serving frontend from EC2 nginx alongside the backend
4. **GitHub push protection blocked secrets** — AWS keys were hardcoded in WORKFLOW.md. Removed, amended commit, force-pushed
5. **"Made with Emergent" badge** — Removed from `index.html` along with associated Emergent script and PostHog analytics

### Summary
Complete AWS free-tier deployment achieved. Single URL (`http://54.206.59.45`) serves both React frontend and FastAPI backend via nginx reverse proxy. Backend auto-restarts on reboot via systemd. Total AWS cost: $0/month.
